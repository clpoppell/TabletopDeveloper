...
public class CharacterRace{
		...
		for(int i=7; i<raceFeatures.length; i++){
			...
			switch(feature[0]){
				...
				case "Weapon Proficiencies":
					racialWeaponTraining= feature[1];
					break;
	...
	public Boolean hasASIChoice(){
		return statMods[1].split(" ")[1].trim().equals("Choice");
	}
	...
	public Map<String,Integer> getStatMods(){
		Map<String,Integer> statModMap= new HashMap<>();
	...
	public String toString(){
		...
		s += ageRange[0] + "-" + ageRange[1] + "\n";
		s += racialSize + "\n";
		s += racialSpeed + "\n";
		
		for(int i=0; i<racialLanguages.length; i++){
			if(i < racialLanguages.length-1){ s += racialLanguages[i] + ", "; }
			else{
				s += racialLanguages[i] + "\n";
				break;
			}
		...
		if(racialSkills != null){ s += racialSkills + "\n"; }
		...
		if(racialWeaponTraining != null){ s += racialWeaponTraining + "\n"; }
